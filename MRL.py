...
class MatryoshkaContrastiveLoss(nn.Module):
    """
    MatryoshkaContrastiveLoss is a custom loss function for contrastive learning
    with Matryoshka Representation, which calculates loss with nesting dims.

    Args:
        relative_importance (List[float]): A list of relative importance weights for each matryoshka dimension term.
    ...
    def _contrastive_loss(self, emb1, emb2):
        ...
        for emb1_matry, emb2_matry in zip(emb1, emb2):
            sim_matrix_matry = F.cosine_similarity(emb1_matry.unsqueeze(1), emb2_matry.unsqueeze(0),
...
class MatryoshkaLinearLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.nesting_dims = [2 ** i for i in range(5, int(math.log2(self.input_dim)) + 1)].append(
            self.input_dim) if nesting_dims is None else nesting_dims
        ...
        for i, nested_dim in enumerate(self.nesting_dims):
            if self.efficient_matryoshka:
                ...
                if projection.bias is None:
                    ...
                    nesting_logits += (
                        torch.matmul(x[:, :nested_dim], (projection.weight[:, :nested_dim]).t()) + projection.bias,)
            else:
